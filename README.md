...
- **彩蛋位设置**面板可调整/删除/新增彩蛋触发位：每行一个彩蛋（人格 + 触发题号 + 选项），可删可改；底部添加入口只列出尚未占用触发位的彩蛋人格，且触发位不可与现有彩蛋重复。删除触发位后该彩蛋人格将无法被玩家获得；
- 「保存到 GitHub」（页面底部）→ 提交到仓库 `data/content.json` → 帽子云检测到 push 自动重建 → **1-3 分钟后全站生效**；
- 保存前会检查彩蛋可达性：触发位选项的 tag 不是该彩蛋人格时，彩蛋将永远无法触发，二次确认（不阻断）；
- 「恢复默认题库」用 `js/data.js` 的出厂题库 + 出厂彩蛋表覆盖线上；「重新加载」丢弃未保存修改。
